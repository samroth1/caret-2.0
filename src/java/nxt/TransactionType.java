    ...
    */
    ...
    public static abstract class CaretMining extends TransactionType {
    	...
    	public static final TransactionType REWARD_RECIPIENT_ASSIGNMENT = new CaretMining() {
    		...
    		@Override
            Attachment.CaretMiningRewardRecipientAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException {
                return new Attachment.CaretMiningRewardRecipientAssignment(buffer, transactionVersion);
            }

            @Override
            Attachment.CaretMiningRewardRecipientAssignment parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException {
            ...
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Set<String>> duplicates) {
                ...
                return isDuplicate(CaretMining.REWARD_RECIPIENT_ASSIGNMENT, Convert.toUnsignedLong(transaction.getSenderId()), duplicates);
            }
    		...
    		void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
    			...
    			if(height < Constants.CARET_REWARD_RECIPIENT_ASSIGNMENT_START_BLOCK) {
    				throw new NxtException.NotCurrentlyValidException("Reward recipient assignment not allowed before block " + Constants.CARET_REWARD_RECIPIENT_ASSIGNMENT_START_BLOCK);
    			}
    		}
		...
		public final static TransactionType ESCROW_CREATION = new AdvancedPayment() {
			...
			void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
				...
				if(transaction.getFeeNQT() < Constants.ONE_NXT) {
					throw new NxtException.NotValidException("Escrow transaction must have a fee at least 1 caret");
				}
		...
		public final static TransactionType SUBSCRIPTION_SUBSCRIBE = new AdvancedPayment() {
			...
			void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
				...
				if(attachment.getFrequency() == null ||
				   attachment.getFrequency().intValue() < Constants.CARET_SUBSCRIPTION_MIN_FREQ ||
				   attachment.getFrequency().intValue() > Constants.CARET_SUBSCRIPTION_MAX_FREQ) {
				...
				if(transaction.getAmountNQT() < Constants.ONE_NXT || transaction.getAmountNQT() > Constants.MAX_BALANCE_NQT) {
					throw new NxtException.NotValidException("Subscriptions must be at least one caret");
				}
				if(transaction.getSenderId() == transaction.getRecipientId()) {
