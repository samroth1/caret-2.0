var NSV = (function(NSV, $, undefined) {	
	...
	NSV.getAccountError = function(accountId, callback) {
		NRS.sendRequest("getAccount", {
			...
			if (response.publicKey) {
				callback({
					"type": "info",
					"message": "The recipient account has a public key and a balance of " + NRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " CARET.",
					"account": response
				});
			} else {
				if (response.errorCode) {
					...
					callback({
						"type": "warning",
						"message": "The recipient account does not have a public key, meaning it has never had an outgoing transaction. The account has a balance of " + NRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " CARET. Please double check your recipient address before submitting.",
						"account": response
					});
				}
			}
	...
	NSV.div_send_calc_dividends = function () {
		...
		if (!(/^\d+$/.test(asset))) {
			...
			NRS.sendOutsideRequest("/caret?requestType=" + "getAsset", {
				"asset": asset
			}, function(response) {
				if (response.errorCode) {
				...
				NRS.sendOutsideRequest("/caret?requestType=" + "getTransaction", {"transaction": asset}, function(response) {				
					if (response.errorCode) {
				...
				NRS.sendOutsideRequest("/caret?requestType=" + "getTime", {}, function(response) {
					if (response.errorCode) {
				...
				if (list_nodist_acc !== "") {
					if (list_nodist_acc !== "ISSUER") {
						...
						for (i=0; i<account_array.length; ++i) {
							var act_acc = account_array[i];
							NRS.sendOutsideRequest("/caret?requestType=" + "getAccount", {
								"account": act_acc
							}, function(response) {
								if (response.errorCode) {
				...
				NRS.sendOutsideRequest("/caret?requestType=" + "getTrades", {
					"asset": asset
				}, function(response) {
					if (response.errorCode) {
						...
						for (var i=0;i<trade_arr.length;i++) {
							if (trade_arr[i].timestamp <= activate_timestamp) {
								...
								NRS.sendOutsideRequest("/caret?requestType=" + "getTransaction", {
	...
	NSV.div_send_get_transactions = function(cur_asset, asset_array,initial_time,final_time) {
		...
		for (var i=0; i<len; i++) {
			...
			NRS.sendOutsideRequest("/caret?requestType=" + "getAccountTransactionIds", {
